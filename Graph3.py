##bibliotecas úteis para o funcionamento do sistema
import pandas as pd
...
def graph3():
    ...
    ##divisão por colunas para melhor visualização
    ...
    ##calcula o período para a seleção dos botões correspondentes
    ...
    ##plica filtro de período selecionado
    df_filtrado = df[(df['dh_recebimento_exame'] >= filtro_periodo[0]) & (df['dh_recebimento_exame'] <= filtro_periodo[1])]

    ##contabiliza a quantidade total de casos
    total_casos = len(df_filtrado)

    ##rankeia as alas por maior número de infecções
    ...
    ##rankeia os microorganismos mais encontrados
    ...
    ##exibição dos resultados obtidos pelos rankeamentos feitos
